...
methods to safely work with array metadata.
...
ensure the metadata is always a valid representation of array metadata.
...
For example, `set`ting array metadata should be done through the
...
methods (or indirectly through the `N5Utils.save` [methods mentioned above](#arrays))
